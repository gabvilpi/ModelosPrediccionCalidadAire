...
X <- datos[datos$contaminante==c, c("estacion","media")]
...
for (i in levels(X$estacion)) {
  X_lagged <- rbind(X_lagged, t(matrix(X[X$estacion==i,"media"], nrow = k, ncol =  as.integer(nrow(X)/k))))
}
...
model %>%
  layer_lstm(units = 6, batch_input_shape = c(batch_size, k-1, 1), 
...
X_train <- data.matrix(X_train[1:num,])

# Redimensionamos la matrix X_train
dim(X_train) <- c(dim(X_train),1)

model %>% fit(X_train, Y_train[1:num], batch_size = batch_size, epochs = epochs, verbose = 1, shuffle = FALSE)
...
X_test<-data.matrix(X_test[1:num,])
