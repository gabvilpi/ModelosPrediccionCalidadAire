...
- Se normalizan todas las medidas mediante *z-score* $$z_i = \frac{x_i - \overline{x}}{\sigma}$$ en un nuevo dataframe "datosNorm"
...
columnas <- c("estacion","fecha","contaminante","hora1","hora2","hora3","hora4","hora5","hora6","hora7","hora8","hora9","hora10","hora11","hora12","hora13","hora14","hora15","hora16","hora17","hora18","hora19","hora20","hora21","hora22","hora23","hora24","media","prec","presMax","presMin","tmax","tmed","tmin","racha","velMedia","laborable")
...
# Seleccionamos el contaminante que deseamos predecir (código numérico)
c = 8

# Determinamos el número k de observaciones a agrupar
k = 7

# Almacenamos las observaciones del contaminante c como series temporales en X
X <- datos[datos$contaminante==c,"media"]

# Creamos un data frame en el que almacenar las observaciones agrupadas
X_lagged <- data.frame()

# Agrupamos las observaciones en grupos de k observaciones consecutivas
for (i in k:length(X)) {
  X_lagged <- rbind(X_lagged, X[(i-(k-1)):i])
}

# Modificamos el nombre de las columnas
colnames(X_lagged) <- c("x1","x2","x3","x4","x5","x6","Y")

# Separamos aleatoriamente los conjuntos de entrenamiento y prueba (80% y 20% respectivamente)
sample<-sample.int(nrow(X_lagged),0.8*nrow(X_lagged))

X_train <- X_lagged[sample,-7]
Y_train <- X_lagged[sample,7]

X_test <- X_lagged[-sample,-7]
Y_test <- X_lagged[-sample,7]

# Borramos el dataframe temporal
rm(X_lagged)
...
model %>%
  layer_lstm(units = 6, batch_input_shape = c(1, 6, 1), 
             return_sequences = TRUE, stateful=TRUE) %>% 
  layer_lstm(units = 6, return_sequences = TRUE, stateful=TRUE) %>% 
  layer_lstm(units = 6, return_sequences = FALSE, stateful=TRUE) %>% 
  layer_dense(units = 1)
...
dim(X_train) <- c(nrow(X_train),6,1)
