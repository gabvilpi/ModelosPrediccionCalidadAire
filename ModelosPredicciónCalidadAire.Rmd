...
X <- datos[datos$estacion==28079050 & datos$contaminante==c,"media"]
...
model %>%
  layer_lstm(units = 6, batch_input_shape = c(batch_size, 6, 1), 
...
# Truncamos las observaciones de X_train y Y_train para coincidir con el batch_size
...
dim(X_train) <- c(dim(prueba),1)

model %>% fit(X_train, Y_train[1:num], batch_size = batch_size, epochs = 15, verbose = 1, shuffle = FALSE)
