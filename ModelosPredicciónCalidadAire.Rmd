...
- Se imputa a los valores perdidos (NA) el valor de la media aritmética del resto de valores observados. Para los datos vacíos de las medidas de calidad del aire, se les imputa la media de los valores recogidos para dicho contaminante en esa misma franja horaria y en el caso de los datos vacíos en las mediciones de climatología, se les imputa la media de los valores recogidos ese mismo día en el resto de años.

```{r imputación_valores, message=FALSE, warning=FALSE}

# Medidas de contaminación: imputamos a los datos vacíos el valor medio recogido para ese contaminante en cada franja horaria
for (i in levels(datos$contaminante)) {
  for (j in levels(datos$estacion)) {
    for (k in colnames(datos)[4:27]) {
      temp<-is.na(datos[[k]])
...
# Medidas de climatología: imputamos a los datos vacíos el valor medio recogido ese mismo día en el resto de años
for (i in colnames(datos)[28:35]) {
    temp<-is.na(datos[[i]])
    unique = unique(datos[temp,2])
    if (length(unique)!=0) {
        for (j in 1:length(unique)) {
            ...
            fecha=unique[j]
            for (k in 2013:2018) {
                replacement = paste0(k, "-")
                fecha=gsub("\\d\\d\\d\\d-*", replacement,fecha)
                valor = datos[datos$fecha==fecha,i][1]
                if (!(is.na(valor))) {
                    suma = suma + valor
					num = num + 1
                }
			...
			media = suma / num
			datos[temp&datos$fecha==unique[j],i]<-media
        }
    }
}
...
Una vez integrados y preparados los datos necesarios para construir el modelo, es conveniente normalizar los datos. La normalización consiste en modificar los datos para lograr que estén en una escala de valores equivalente, de forma que se simplifique la comparación entre ellos. Si los datos no se normalizan, los métodos de minería de datos que se utilicen tenderán a quedar sesgados debido a la influencia de los atributos con valores más altos, distorsionando el resultado del modelo.

En este caso se hará uso de el método de "Normalización por la diferencia". Este método intenta compensar el efecto que provoca la distancia del valor que se trata con respecto al máximo de los valores observados. Su fórmula de cálculo es:

$$z_i = \frac {x_i - x_{min}} {x_{max} - x_{min}} $$

A continuación preparamos una función para normalizar los datos antes de entrenar nuestro modelo. La función tendrá como entrada una lista con los datos X e Y seleccionados para entrenar o testear el modelo. La función se encargará de normalizar los datos mediante la técnica de "Normalización por la diferencia" y devolverá los datos X e Y normalizados y los valores máximo y mínimo para que sea posible revertir el proceso desnormalizando los datos.
...
Por último, se establece una función para preparar los datos de entrenamiento y test con los que alimentar y probar los modelos. 

La función tiene como entrada el código del contaminante seleccionado para el análisis, el código de la estación meteorológica de la que seleccionar los datos y el número de predictores a utilizar en la predicción (1: Sólo medidas de contaminación, 2: Medidas de contaminación y climatología y 3: Medidas de contaminación, climatología y calendario laboral).

Las observaciones del contaminante se almacenan como series temporales en X e Y, es decir, el valor de Y[i] se corresponde a la siguiente medición a X[i]. Para realizar la predicción de los niveles de alta contaminación, se ha seleccionado la medida máxima diaria recogida para el contaminante y estación seleccionados.

Los sets de entrenamiento y test se dividen en una proporción del 80% y 20% respectivamente. Después se normalizan todos los datos mediante la función creada anteriormente y se devuelven en una lista que contiene X_test, Y_test, X_train e Y_train y sus respectivos máximos y mínimos para que sea posible revertir la normalización (maxXTrain, maxXTest, maxYTrain, maxYTest, minXtrain, minXTest, minYTrain, minYTest).

```{r preparar_datos, message=FALSE, warning=FALSE}

preparar_datos <- function(c, estacion, predictores, pred_clima) {
  ...
  # pred_clima: TRUE: Se usan como variables predictoras los datos del clima del día en que se quiere predecir la contaminación (uso de predicción del clima)
  # pred_clima: FALSE: Se usan como variables predictoras los datos del clima del día anterior al que se quiere predecir la contaminación
  ...
  if (is.null(pred_clima)) stop("El parámetro pred_clima es obligatorio")
  ...
  if (predictores == 1) {
    ...
    if (pred_clima==TRUE) {
      ...
      # Almacenamos las observaciones del contaminante c como series temporales en X e Y, siendo Y[i,] la siguiente medición a X[i,]
      # Almacenamos en X las mediciones climatológicas del mismo día en que deseamos predecir la contaminación (uso de predicción del clima)
      X1 <- datos[datos$contaminante==c&datos$estacion==estacion,"maximo"]
      X2 <- datos[datos$contaminante==c&datos$estacion==estacion,c("prec","presMax","presMin","tmax","tmed","tmin", "racha","velMedia")]
      ...
      # Almacenamos las observaciones del contaminante c como series temporales en X e Y, siendo Y[i,] la siguiente medición a X[i,]
      # Almacenamos en X las mediciones climatológicas del día anterior al que deseamos predecir la contaminación
    ...
    if (pred_clima==TRUE) {
      ...
      # Almacenamos en X las mediciones climatológicas del mismo día en que deseamos predecir la contaminación (uso de predicción del clima)
      X1 <- datos[datos$contaminante==c&datos$estacion==estacion,"maximo"]
      X2 <- datos[datos$contaminante==c&datos$estacion==estacion,c("prec","presMax","presMin","tmax","tmed","tmin", "racha","velMedia","laborable")]
      ...
      # Almacenamos las observaciones del contaminante c como series temporales en X e Y, siendo Y[i,] la siguiente medición a X[i,]
      # Almacenamos en X las mediciones climatológicas del día anterior al que deseamos predecir la contaminación
      X1 <- datos[datos$contaminante==c&datos$estacion==estacion,c("maximo","prec","presMax","presMin","tmax","tmed","tmin", "racha","velMedia")]
      X2 <- datos[datos$contaminante==c&datos$estacion==estacion,"laborable"]
...
## 2. Caracterización del clima de Madrid
...
print(colMeans(datos[datos$estacion==28079004&format(datos$fecha,"%m-%d")>='03-20'&format(datos$fecha,"%m-%d")<='06-20',c("prec","presMax","presMin","racha","tmax","tmed","tmin","velMedia")], na.rm = TRUE))

# verano (21/06 - 22/09)
cat("\nclimatología en los meses de verano:\n")
print(colMeans(datos[datos$estacion==28079004&format(datos$fecha,"%m-%d")>='06-21'&format(datos$fecha,"%m-%d")<='09-22',c("prec","presMax","presMin","racha","tmax","tmed","tmin","velMedia")], na.rm = TRUE))

# otoño (23/09 - 20/12)
cat("\nClimatología en los meses de otoño:\n")
print(colMeans(datos[datos$estacion==28079004&format(datos$fecha,"%m-%d")>='09-23'&format(datos$fecha,"%m-%d")<='12-20',c("prec","presMax","presMin","racha","tmax","tmed","tmin","velMedia")], na.rm = TRUE))

# invierno (21/12 - 19/03)
cat("\nClimatología en los meses de invierno:\n")
print(colMeans(datos[datos$estacion==28079004&(format(datos$fecha,"%m-%d")>='12-21'&format(datos$fecha,"%m-%d")<='12-31') | (format(datos$fecha,"%m-%d")>='01-01' & format(datos$fecha,"%m-%d")<='03-19'),c("prec","presMax","presMin","racha","tmax","tmed","tmin","velMedia")], na.rm = TRUE))
...
## 3. Modelo de predicción de la calidad del aire de Madrid

```{r cargar_y_preparar_datos, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}

# Este bloque carga el fichero "datos.csv" con los datos ya preprocesados y las funciones preparar_datos y normalizar
...
Funciones genéricas de entrenamiento y predicción para los modelos MLP, LSTM y CNN. Además de funciones de evaluación y comparación de resultados para todos los modelos:

```{r entrenar_modelo, message=FALSE, warning=FALSE}

entrenar_modelo <- function(tipo, modelo, batch_size, epochs) {
...
```{r predicciones_modelo, message=FALSE, warning=FALSE}

predicciones_modelo <- function(tipo, modelo, batch_size) {
...
```{r evaluar_modelo, message=FALSE, warning=FALSE}
...
comparar_prediccion<-function(Y_test,Y_pred, titulo) {
  ...
  colnames(resultado) <- c("Y_test","Y_pred")
  
  # Creamos un vector X con para el eje de abscisas
  X <- c(1:length(Y_pred))
  
  # Mostramos los datos reales y las predicciones con un gráfico de líneas
  p <- ggplot() + geom_line(data = resultado, aes(x=X, y=resultado$Y_test, colour = "Y_test"), size = 1) + geom_line(data = resultado, aes(x=X, y=resultado$Y_pred, colour = "Y_pred"), size = 1) + xlab("Tiempo") + ylab("Contaminación") + ggtitle(titulo)
...
### 3.1. Modelo basado en MLP (Multilayer perceptron)

```{r crear_modelo_mlp, message=FALSE, warning=FALSE}

crear_modelo_mlp <- function(batch_size, n, units, activation) {
  ...
  modelo %>%
    layer_dense(units = units, batch_input_shape = c(batch_size, n), activation = activation) %>% 
...
# Este bloque realiza un benchmark del modelo MLP para encontrar los mejores hiperparámetros en base al RMSE

tipo = "mlp" # Modelo tipo MLP
...
pred_clima = FALSE # Uso de predicción del clima
activation_valores = c("softmax","relu","tanh") # Activadores a evaluar
...
for (k in predictores) {
  ...
  for (activation in activation_valores) {
    for (batch_size in batch_size_valores) {
      for (units in units_valores) {
        for (epochs in epochs_valores) {
          
          cat("Evaluando modelo MLP con c:",c,"predictores:",k,"activation:",activation,"batch_size:",batch_size,"units:",units,"epochs:",epochs,"\n")
          
          # Preparamos los datos de entrenamiento y prueba
          returnList <- preparar_datos(c, estacion, k, pred_clima)
          ...
          modelo <- crear_modelo_mlp(batch_size, n, units, activation)
          
          # Entrenamos el modelo con X_train e Y_train (variables globales)
          entrenar_modelo(tipo, modelo, batch_size, epochs)
          
          # Predecimos los resultados para X_test (variable global)
          Y_pred <- predicciones_modelo(tipo, modelo, batch_size)
          ...
          titulo = paste("Modelo MLP, predictores:",k,"\nactivation:",activation,"batch_size:",batch_size,"units:",units,"epochs:",epochs,"\nRMSE:",rmse)
          comparar_prediccion(Y_test,Y_pred,titulo)
          
          # Almacenamos los resultados de los hiperparámetros evaluados en el fichero "mlp_resultados_bench.csv"
          newRow <- data.frame(c=c, k=k, activation=activation,batch_size=batch_size, units=units, epochs=epochs, rmse=rmse)
          resultados <- rbind(resultados, newRow)
          rm(newRow)
...
```{r modelo_mlp, message=FALSE, warning=FALSE}

# Establecemos los hiperparámetros del modelo MLP
tipo = "mlp" # Modelo tipo MLP
...
cat("Predicción mediante modelo MLP.\n Contaminante:",cod_contaminante,"predictores:",predictores,"activation:",activation,"batch_size:",batch_size,"units:",units,"epochs:",epochs,"\n")

# Preparamos los datos de entrenamiento y prueba
returnList <- preparar_datos(cod_contaminante, cod_estacion, predictores, pred_clima)
...
entrenar_modelo(tipo, modelo, batch_size, epochs)
end_time <- Sys.time()
timeMLP = end_time - start_time
cat("Tiempo total de entrenamiento:",timeMLP,"\n")
...
# Comparamos los datos reales y las predicciones con un gráfico de líneas 
titulo = paste("Modelo MLP, predictores:",predictores,"\nactivation:",activation,"batch size:",batch_size,"units:",units,"epochs:",epochs,"\nRMSE:",rmse)
comparar_prediccion(Y_test,Y_pred,titulo)

```

### 3.2. Modelo basado en LSTM (Long Short-Term Memory)

```{r crear_modelo_lstm, message=FALSE, warning=FALSE}

crear_modelo_lstm <- function(batch_size, n, units, activation) {
  ...
  modelo %>%
    layer_lstm(units = units, batch_input_shape = c(batch_size, 1, n), 
               return_sequences = TRUE, stateful=TRUE, activation = activation) %>% 
    layer_lstm(units = units, return_sequences = TRUE, stateful=TRUE, activation = activation) %>% 
    layer_lstm(units = units, return_sequences = FALSE, stateful=TRUE, activation = activation) %>% 
    layer_dense(units = 1)
...
# Este bloque realiza un benchmark del modelo LSTM para encontrar los mejores hiperparámetros en base al RMSE

tipo = "lstm" # Modelo tipo LSTM
c = 8 # Dióxido de nitrógeno
estacion = 28079004 # Plaza de España
predictores = c(1,2,3) # Numero de variables predictoras a considerar
pred_clima = FALSE # Uso de predicción del clima
    ...
    for (batch_size in batch_size_valores) {
      for (units in units_valores) {
        for (epochs in epochs_valores) {
          ...
          cat("Evaluando modelo LSTM con c:",c,"predictores:",k,"activation:",activation,"batch_size:",batch_size,"units:",units,"epochs:",epochs,"\n")
          ...
          modelo <- crear_modelo_lstm(batch_size, n, units, activation)
          ...
          # Comparamos los datos reales y las predicciones con un gráfico de líneas 
          titulo = paste("Modelo LSTM, predictores:",k,"\nactivation",activation,"batch_size:",batch_size,"units:",units,"epochs:",epochs,"\nRMSE:",rmse)
          comparar_prediccion(Y_test,Y_pred,titulo)
...
cat("Tiempo total de entrenamiento:",timeLSTM,"\n")
...
# Comparamos los datos reales y las predicciones con un gráfico de líneas 
titulo = paste("Modelo LSTM, predictores:",predictores,"\nactivation:",activation,"batch size:",batch_size,"units:",units,"epochs:",epochs,"\nRMSE:",rmse)
comparar_prediccion(Y_test,Y_pred,titulo)

```

### 3.3. Modelo basado en CNN (Convolutional Neural Network)

```{r crear_modelo_cnn, message=FALSE, warning=FALSE}

crear_modelo_cnn <- function(batch_size, n, filters, activation) {
  ...
  # Añadimos las capas al modelo
  modelo %>%
    layer_conv_1d(filters = filters, kernel_size = 1, activation = activation, batch_input_shape = c(batch_size, 1, n)) %>%
    layer_conv_1d(filters = filters, kernel_size = 1, activation = activation) %>% 
...
```{r benchmark_modelo_cnn, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}

# Este bloque realiza un benchmark del modelo CNN para encontrar los mejores hiperparámetros en base al RMSE

tipo = "cnn" # Modelo tipo CNN
c = 8 # Dióxido de nitrógeno
estacion = 28079004 # Plaza de España
predictores = c(1,2,3) # Numero de variables predictoras a considerar
pred_clima = FALSE # Uso de predicción del clima
activation_valores = c("softmax","relu","tanh") # Activadores a evaluar
batch_size_valores = c(1,7,30) # batch_sizes a evaluar
filters_valores = c(1,8,16,32,64) # Numero de filters a evaluar
epochs_valores = c(1,5,10) # Número de épocas a evaluar
...
for (k in predictores) {
  ...
  for (activation in activation_valores) {
    for (batch_size in batch_size_valores) {
      for (filters in filters_valores) {
        for (epochs in epochs_valores) {
          
          cat("Evaluando modelo CNN con c:",c,"predictores:",k,"activation:",activation,"batch_size:",batch_size,"filters:",filters,"epochs:",epochs,"\n")
          ...
          modelo <- crear_modelo_cnn(batch_size, n, filters, activation)
          ...
          # Comparamos los datos reales y las predicciones con un gráfico de líneas 
          titulo = paste("Modelo CNN, predictores:",k,"\nactivation",activation,"batch_size:",batch_size,"filters:",filters,"epochs:",epochs,"\nRMSE:",rmse)
          comparar_prediccion(Y_test,Y_pred,titulo)
          
          # Almacenamos los resultados de los hiperparámetros evaluados en el fichero "lstm_resultados_bench.csv"
          newRow <- data.frame(c=c, k=k, activation=activation, batch_size=batch_size, filters=filters, epochs=epochs, rmse=rmse)
          resultados <- rbind(resultados, newRow)
          rm(newRow)
          write.csv2(resultados, file = "cnn_resultados_bench.csv", row.names= FALSE, col.names = TRUE)
...
```{r modelo_cnn, message=FALSE, warning=FALSE}

# Establecemos los hiperparámetros del modelo CNN
tipo = "cnn" # Modelo tipo LSTM
cod_contaminante = 8 # Dióxido de nitrógeno
cod_estacion = 28079004 # Plaza de España
predictores = 3 # Como variables predictoras utilizamos las medidas de contaminación, las medidas climatológicas y el calendario laboral
pred_clima = FALSE # Uso de predicción del clima
...
cat("Predicción mediante modelo CNN\n Contaminante:",cod_contaminante,"predictores:",predictores,"activation:",activation,"batch_size:",batch_size,"filters:",filters,"epochs:",epochs,"\n")
...
entrenar_modelo(tipo, modelo, batch_size, epochs)
end_time <- Sys.time()
timeCNN = end_time - start_time
cat("Tiempo total de entrenamiento:",timeCNN,"\n")
...
# Comparamos los datos reales y las predicciones con un gráfico de líneas 
titulo = paste("Modelo CNN, predictores:",predictores,"\nactivation:",activation,"batch size:",batch_size,"filters:",filters,"epochs:",epochs,"\nRMSE:",rmse)
comparar_prediccion(Y_test,Y_pred,titulo)

```

### 3.4. Modelo basado en SVM (Support Vector Machines)

```{r benchmark_modelo_svm, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}

# Este bloque realiza un benchmark del modelo SVM para encontrar los mejores hiperparámetros en base al RMSE

# Cargamos la librería e1071
library(e1071)

# Establecemos los hiperparámetros del modelo SVM
c = 8 # Dióxido de nitrógeno
cod_estacion = 28079004 # Plaza de España
predictores = c(1,2,3) # Como variables predictoras utilizamos las medidas de contaminación, las medidas climatológicas y el calendario laboral
pred_clima = FALSE # Uso de predicción del clima
type_valores = c("eps-regression", "nu-regression")
kernel_valores = c("linear","polynomial","radial","sigmoid")

resultados <- data.frame(stringsAsFactors = FALSE)
        
for (k in predictores) {
  for (type in type_valores) {
    for (kernel in kernel_valores) {
      
      cat("Predicción mediante modelo SVM\n Contaminante:",cod_contaminante,"predictores:",k,"type:",type,"kernel:",kernel,"\n")

      # Preparamos los datos de entrenamiento y prueba
      returnList <- preparar_datos(cod_contaminante, cod_estacion, k, pred_clima)
      ...
      # Creamos el modelo SVM con X_train e Y_train (variables globales)
      modelo <- svm(x = X_train, y = Y_train, type = type, kernel = kernel)
      ...
      # Comparamos los datos reales y las predicciones con un gráfico de líneas 
      titulo = paste("Modelo SVM, predictores:",k,"\ntype:",type,"kernel:",kernel,"RMSE:",rmse)
      comparar_prediccion(Y_test,Y_pred,titulo)
      
      # Almacenamos los resultados de los hiperparámetros evaluados en el fichero "svm_resultados_bench.csv"
      newRow <- data.frame(c=c, k=k, type = type, kernel = kernel, rmse = rmse)
      resultados <- rbind(resultados, newRow)
      rm(newRow)
      write.csv2(resultados, file = "svm_resultados_bench.csv", row.names= FALSE, col.names = TRUE)
...
```{r modelo_svm, message=FALSE, warning=FALSE}
...
cat("Predicción mediante modelo SVM\n Contaminante:",cod_contaminante,"predictores:",predictores,"type:",type,"kernel:",kernel,"\n")
...
modelo <- svm(x = X_train, y = Y_train, type = type, kernel = kernel)
end_time <- Sys.time()
timeSVM = end_time - start_time
cat("Tiempo total de entrenamiento:",timeSVM,"\n")
...
Y_pred <- data.matrix(Y_pred)

# Comparamos los datos reales y las predicciones con un gráfico de líneas 
titulo = paste("Modelo SVM, predictores:",predictores,"\ntype:",type,"kernel:",kernel,"RMSE:",rmse)
comparar_prediccion(Y_test,Y_pred,titulo)
