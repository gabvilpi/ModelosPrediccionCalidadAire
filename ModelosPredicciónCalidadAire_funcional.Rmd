...
crear_modelo_cnn <- function(batch_size, n, filters) {
  ...
  # Añadimos las capas al modelo
  modelo %>%
    layer_conv_1d(filters = filters, kernel_size = 1, activation = "softmax", batch_input_shape = c(batch_size, 1, n)) %>%
    layer_conv_1d(filters = filters, kernel_size = 1, activation = "softmax") %>% 
  ...
  for (batch_size in batch_size_valores) {
    for (filters in filters_valores) {
      for (epochs in epochs_valores) {
        ...
        cat("Evaluando modelo CNN con c:",c,"predictores:",k,"batch_size:",batch_size,"filters:",filters,"epochs:",epochs,"\n")
        ...
        modelo <- crear_modelo_cnn(batch_size, n, filters)
        ...
        # Almacenamos los resultados de los hiperparámetros evaluados en el fichero "lstm_resultados_bench.csv"
        resultados <- rbind(resultados, c(c,k,batch_size,filters,epochs,rmse))
        colnames(resultados) <- c("c","k","batch_size","filters","epochs","rmse")
...
cat("Predicción mediante modelo CNN\n Contaminante:",cod_contaminante,"predictores:",predictores,"batch_size:",batch_size,"filters:",filters,"epochs:",epochs,"\n")
