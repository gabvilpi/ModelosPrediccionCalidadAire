...
comparar_prediccion<-function(Y_test,Y_pred) {
  ...
  ggplot() + geom_line(data = resultado, aes(x=X, y=resultado$V1, colour = "Y_test"), size = 1) + geom_line(data = resultado, aes(x=X, y=resultado$V2, colour = "Y_pred"), size = 1) + xlab("Tiempo") + ylab("Contaminación")
  ...
  for (batch_size in batch_size_valores) {
    for (units in units_valores) {
      for (epochs in epochs_valores) {
        ...
        print(comparar_prediccion(Y_test,Y_pred))
