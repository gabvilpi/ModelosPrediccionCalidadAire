...
preparar_datos <- function(c, estacion, predictores, objetivo) {
  ...
  # objetivo: (variable objetivo) número de días a predecir
  ...
  if (is.null(objetivo)) stop("El parámetro objetivo es obligatorio.")
  ...
  # Modificamos Y_train e Y_test según el número de días a predecir
  if (objetivo > 1) {
    ...
    for (o in 2:objetivo) {
      ...
      if (o == 2) {
        ...
        Y_train <- cbind(Y_train,c(Y_train[-1],NA))
        Y_test <- cbind(Y_test,c(Y_test[-1],NA))
        
      } else {
        
        Y_train <- cbind(Y_train,c(Y_train[-1,ncol(Y_train)],NA))
        Y_test <- cbind(Y_test,c(Y_test[-1,ncol(Y_test)],NA))
        
      }
      
    }
    
    # Recortamos las columnas necesarias tanto en X como en Y
    Y_train <- Y_train[1:(nrow(Y_train)-(objetivo-1)),]
    Y_test <- Y_test[1:(nrow(Y_test)-(objetivo-1)),]
    
    if (typeof(X_train) == "list") X_train <- X_train[1:(nrow(X_train)-(objetivo-1)),]
    else X_train <- X_train[1:(length(X_train)-(objetivo-1))]
    
    if (typeof(X_test) == "list") X_test <- X_test[1:(nrow(X_test)-(objetivo-1)),]
    else X_test <- X_test[1:(length(X_test)-(objetivo-1))]
...
normalizar <- function(dataList) {
  ...
  if (typeof(dataList$X_train) == "list") {
    ...
    # Calculamos el máximo y el mínimo del predictor para proceder a la normalización
  ...
  if (typeof(dataList$Y_train) == "list") {
    
    # Y_train e Y_test contienen varios días de predicción en la variable objetivo
    
    # Calculamos los máximos y mínimos
    dataList$maxYTrain = max(c(dataList$Y_train))
    dataList$minYTrain = min(c(dataList$Y_train))
    dataList$maxYTest = max(c(dataList$Y_test))
    dataList$minYTest = min(c(dataList$Y_test))
    ...
    # Y_train e Y_test contienen un único día en la variable objetivo
    
    # Calculamos el máximo y el mínimo de la variable objetivo para proceder a la normalización
...
crear_modelo_lstm <- function(batch_size, units, objetivo) {
...
objetivo = c(1,3,5)
batch_size_valores = c(1,7,30)
units_valores = c(1,8,16,32)
epochs_valores = c(1,5,10)
    ...
    for (batch_size in batch_size_valores) {
      for (units in units_valores) {
        for (epochs in epochs_valores) {
          ...
          cat("Evaluando modelo LSTM con c:",c,"predictores:",k,"días objetivo:",o,"batch_size:",batch_size,"units:",units,"epochs:",epochs,"\n")
          
          returnList <- preparar_datos(c, estacion, k, o)
          ...
          modelo <- crear_modelo_lstm(batch_size, units, o)
          ...
          resultados <- rbind(resultados, c(c,k,o,batch_size,units,epochs,rmse))
          colnames(resultados) <- c("c","k","o","batch_size","units","epochs","rmse")
